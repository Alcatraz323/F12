...
public class AndroidTreeView
{
	...
	private TwoDScrollView cur;
	private List<TreeNode> highlighted;
    ...
    public AndroidTreeView(Context context, TreeNode root)
	{
        mRoot = root;
        mContext = context;
		highlighted=new LinkedList<TreeNode>();
    }
	...
	public List<TreeNode> getHighLighted(){
		return highlighted;
	}
	public void clearHighLight(){
		for(TreeNode h:highlighted){
	...
	{
		String[] index=path.split(TreeNode.NODES_ID_SEPARATOR);
		Log.e("Alc-trav", path + "__size:" + index.length);
		setN(index.length - 1, mRoot, index);
		
	}
	private void setN(int in, TreeNode n, String[] index_p)
	{
		if (in >= 0)
		{
			int indrc=Integer.valueOf(index_p[in]);
			indrc--;
			TreeNode cur=n.getChildren().get(indrc);
			cur.toggleArr();
			if (in == 0)
			{
				cur.setHi("#C3C3C3");
    ...
    private void addNode(ViewGroup container, final TreeNode n)
	{
		...
		ViewGroup vp=(ViewGroup) nodeView.getParent();
		if (vp != null)
		{
			vp.removeView(nodeView);
		}
